...
The third leap went to the metal. I wanted to see whether agents could build something I had never touched: assembler beyond a college lab and DirectX I had skipped entirely. I asked GPT-5 for a DirectX 11 renderer with smooth Mandelbrot and Julia animations, real-time palette switching, and precise zoom control. It replied with a hybrid GPU/CPU plan that felt like a touring bike with an e-assist.

GPU first: a D3D11 compute shader writes the fractal straight into a UAV texture. It picks double precision for deep desktop zooms and falls back to float on ARM when doubles are missing. A fullscreen triangle and pixel shader then blend in a procedural starfield plus a lightweight 2D smoke sim that lifts off the fractal boundary—visual proof the GPU is awake and caffeinated. A Direct2D overlay draws the HUD and help panel into its own texture, then a tiny pixel shader alpha-blends it over the final frame so text stays crisp.

CPU as a backup: a tiled work-stealing renderer fans out tiles to worker threads. A NASM AVX2/FMA kernel processes eight pixels per iteration, handles Mandelbrot and Julia, broadcasts constants into YMM registers, and uses fused multiply-add for the hot loop. Tail pixels that cannot fill a SIMD width fall back to scalar, and a reference C path covers float64 when the assembly kernel sits out. It is the same picture, painted with a different brush when the GPU is busy.

Palettes are precomputed 256-entry BGRA8 tables with smoothstep gradients and gamma-aware blending. Switching palettes updates a structured buffer—no shader recompile, no drama. Zoom uses logarithmic scale interpolation with smoothstep easing so the camera glides instead of lurching; pan follows the mouse, and click-to-zoom lands on the cursor like it knows where you pointed.

Fullscreen was the fussy part: defer resize messages, unbind every SRV, UAV, and RTV before `ResizeBuffers`, and guard against re-entrant resize storms from the window manager. The agent generated the D3D11 boilerplate, the compute shader, the HLSL postprocess bits, the AVX2 assembly kernel, and the resize-safety patterns. My role was to run it, describe what broke, and paste logs. The agent even set up a log routine so I could drop errors straight into the prompt; later it just fetched the log file when I said, “it crashed.” I never knew why it failed, but the agent did—and fixed it. The result hits 60 fps through deep zooms and keeps its sense of humor while the smoke keeps rising.
