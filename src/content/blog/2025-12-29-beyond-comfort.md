...
The pipeline now scans a raw folder, renders waveforms, normalizes, compresses, cuts, applies fades, and exports to the target game sound folder without touching the originals. The agent cached intermediate buffers so tweaks rerun in milliseconds. A file watcher reruns the pipeline when new sounds land in the raw folder, turning the app into a quiet assistant during match days. It behaves like a native tool because it is one, and I did not type the C++.

Trust came from guided inputs. I pulled a batch of sounds from Pixabay after the agent produced a table of search prompts for each needed clip. I downloaded, dropped them into the raw folder, and let Woosh process them. I listened, noted what felt off, and asked the agent for tweaks. That loop—prompt, download, process, listen—was enough to get the set ready for Foosh.
...
The third leap went to the metal. I wanted smooth Mandelbrot and Julia animations and I did not want the GPU driver to guess my intent. I asked GPT-5 for a DirectX renderer that kept camera moves smooth and let me swap coloring schemes. It produced a clean command buffer, a small state struct for fractal parameters, and an entry point that handled window events.

I then asked for the inner iteration loop in assembler with SIMD intrinsics. The first output was wrong; it had branch divergence and alignment mistakes. I asked for instrumentation with cache miss counters and instruction counts, and the agent added timing hooks plus buffer alignment. Aligning the pixel buffer to SIMD widths removed stalls. Moving to fused multiply-add trimmed cycles per iteration. Precomputed palette gradients reduced GPU state churn. Frame time dropped enough to keep 60 fps while flying through deep zooms, and the motion stayed stable when switching to Julia sets mid-flight. I reviewed the assembler for safety and clarity, but the agent did the heavy lifting.

The surprises here were educational. The agent hallucinated an instruction once, and the build failed. That failure was obvious because the instrumentation wrapped the loop, so I saw where it broke. When I asked it to simplify, it rewrote the loop with fewer branches and fewer assumptions about the GPU. The agent works best when told to minimize cleverness and to show metrics. The more I framed success as numbers—frame time targets, cache miss budgets—the more the output matched what I wanted.
